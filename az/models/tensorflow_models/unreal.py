...
class PolicyValueNet(NeuralNetBase):
    ...
    def create_network(self, **kwargs):
        print("PolicyValueNet begin to create network")
        defaults = {
            "board": 19,
        ...
        self.X_ = tf.placeholder(tf.float32, [None, self.board, self.board, self.input_dim], name='X_')
        self.Y_ = tf.placeholder(tf.float32, [None, self.board * self.board + 1], name='Y_')
        self.Z_ = tf.placeholder(tf.float32, [None, 3], name='z_')

        self.training = tf.placeholder(tf.bool, name='training_')
        ...
        self.value_loss = tf.reduce_mean(tf.squared_difference(self.Z_, self.Z_conv))
        ...
        loss = 1.0 * self.value_loss + 1.0 * self.policy_loss + self.reg_term
    ...
    def estimate_value(self, X):
        return self.session.run(tf.reduce_mean(self.Z_conv), feed_dict={self.training: False, self.X_: X})

    def estimate_policy(self, X):
        return self.session.run(tf.nn.softmax(self.Y_conv), feed_dict={self.training: False, self.X_: X})
    ...
    def train_model(self, X, Y, Z):
        ...
        return policy_loss, mes_loss,reg_term,_
    ...
    def process(self, next_batches, model_json,steps):
        print("pvnet_process ing!!")
        ...
        print("--avg_policy_loss:",self.avg_policy_loss,"--avg_value_loss:",self.avg_value_loss,"--avg_reg_term:",self.avg_reg_term,"--steps:",steps)
        ...
        if steps % 320 == 0:
            ...
            print("step:",steps,"--policy loss:",self.avg_policy_loss,"--avg_mse_loss:",self.avg_mse_loss,"--avg_reg_term:",self.avg_reg_term,"speed:",speed)
    ...
    def construct_net(self, planes):
        ...
        # X - SHAPE: (?, 3, 9, 9)
        # W - SHAPE: (3, 3, 3, 128)
        x_planes = tf.reshape(planes, [-1, self.board, self.board, self.input_dim])
        # Input convolution
        ...
        return h_fc1, tf.nn.softmax(h_fc3)
