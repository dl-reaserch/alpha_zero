...
class NeuralNetBase(object):
    ...
    def __init__(self,feature_list,**kwargs):
        ...
        kwargs["input_dim"] = self.preprocessor
    ...
    def _model_forward(self):
        """Construct a function using the current keras_models backend that, when given a batch
                ...
                c.f. https://github.com/fchollet/keras_models/issues/1426
                """
    ...
    def load_model(json_file):
        ...
        try:
            network_class = NeuralNetBase.subclass[class_name]
        except KeyError:
            raise ValueError("Unknown neural network type in json file: {}\n"
                             "(was it registered with the @neuralnet decorator?)"
                             .format(class_name))
        # create new object
        new_net = network_class(object_specs['feature_list'],init_work=False)

        new_net.model = model_from_json(object_specs['keras_models'],custom_objects={'Bias':Bias})
    ...
    def save_model(self,json_file,weights_file=None):
        ...
        object_specs ={
            ...
            'keras_models':self.model.to_json(),
...
def neuralnet(cls):
     NeuralNetBase.subclass[cls.__name__] = cls
     return cls


class Bias(Layer):
    """Custom keras_models layer that simply adds a scalar bias to each location in the input

       Largely copied from the keras_models docs:
       http://keras_models.io/layers/writing-your-own-keras_models-layers/#writing-your-own-keras_models-layers
       """
